...
@php
    ...
    $hasToggleOnlyChildItems = filled($childItems) && (($url === '#') || blank($url));
    $isExpandedByDefault = $active || $activeChildItems;
@endphp

<li
    @if ($hasToggleOnlyChildItems)
        x-data="{ expanded: @js($isExpandedByDefault) }"
    @endif
    {{
        $attributes->class([
            'fi-sidebar-item',
            ...
            'fi-sidebar-item-has-url' => filled($url) || $hasToggleOnlyChildItems,
        ])
    }}
>
    <a
    ...
    @if ($childItems)
        <ul
            @if ($hasToggleOnlyChildItems)
                x-cloak
                x-show="expanded || @js($activeChildItems)"
            @elseif (! ($active || $activeChildItems))
                style="display: none;"
            @endif
